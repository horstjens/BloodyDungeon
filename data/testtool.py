...
class Viech(object):
    ...
    def show(self):
        txt=""
        atts=[x for x in dir(self) if not "__" in x and not "show" in x and not "book" in x]
        for a in atts:
            txt+= "{}: {}\n".format(a, self.__getattribute__(a))
        print(txt)
        return txt
        
yannik=Viech(hp=10, att=10, df=10, dmg=10, arm=5, cri=0.1)
yannik.show()
niklas=Viech(hp=10, att=10, df=10, dmg=10, arm=10, cri=0.2)
niklas.show()
# 0.1 cri = 4 att
# 0.1 cri = 2 df
# 
